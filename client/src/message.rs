...
pub fn process_msg(
    ...
    match msg {
        ...
        Rcvr::Logout(s) => {
            state.buffered_messages.push(s);
            state.running = false;
        }
        ...
        Rcvr::Misc {
            ...
            "join" => {
                ...
                let mut sl = Line::default();
                sl.push("* ");
                if name.as_str() == state.username.as_str() {
                    sl.pushf("You", &BOLD);
                    sl.push(" joined ");
            ...
            "name" => {
                ...
                let mut sl = Line::default();
                sl.push("* ");
                if old.as_str() == state.username.as_str() {
                    sl.pushf("You", &BOLD);
        ...
        if *char == state.cmd {
            ...
            match cmd.as_str() {
                ...
                "unblock" => match split_command_tokens(&cmd_toks, 1) {
                    Ok((_, arg)) => {
                        state.enqueue(&Sndr::Unblock(&arg));
                    }
                    Err(_) => {
                        return;
                    }
