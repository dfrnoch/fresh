...
fn command_key(event: event::KeyEvent, screen: &mut Screen, client_state: &mut ClientState) {
    match client_state.mode {
        Mode::Command => match event.code {
            KeyCode::Char(' ') | KeyCode::Enter => client_state.mode = Mode::Insert,
            ...
            KeyCode::Char('a') => {
                ...
                client_state.mode = Mode::Insert;
            }
            KeyCode::Char('i') => client_state.mode = Mode::Insert,
            ...
            KeyCode::Char('q') => client_state.enqueue(&Sndr::Logout("[ client quit ]")),
            KeyCode::Char('d') => client_state.mode = Mode::Delete,
            _ => {}
        },
        Mode::Delete => match event.code {
            KeyCode::Char('h') => {
                screen.input_skip_chars(-1);
                // screen.delete_char();
                client_state.mode = Mode::Command;
            }
            KeyCode::Char('l') => {
                screen.input_skip_chars(1);
                // scrn.delete_char();
...
fn input_key(event: event::KeyEvent, screen: &mut Screen, client_state: &mut ClientState) {
    match event.code {
        KeyCode::Enter => respond_to_user_input(screen.pop_input(), screen, client_state),
        KeyCode::Backspace => {
...
pub fn process_user_typing(
    ...
    while event::poll(Duration::default())? {
        let prev_mode = client_state.mode;

        if let Ok(Event::Key(event)) = event::read() {
            trace!("event: {:?}", event);
            match client_state.mode {
                Mode::Command | Mode::Delete => command_key(event, screen, client_state),
                Mode::Insert => input_key(event, screen, client_state),
            }
        } else if let Ok(Event::Resize(w, h)) = event::read() {
            screen.resize(w, h);
        }

        if prev_mode != client_state.mode {
...
pub fn write_mode_line(screen: &mut Screen, client_state: &ClientState) {
    let mut mode_line = Line::default();
    let mch: &str = match client_state.mode {
        Mode::Insert => "Ins",
        Mode::Command => "Com",
        Mode::Delete => "Del",
    };
    ...
    mode_line.pushf(" │ ", &DIM);
    mode_line.pushf(&(client_state.username), &HIGHLIGHT);
    mode_line.push(" @ ");
    mode_line.pushf(&(client_state.local_address), &HIGHLIGHT);
