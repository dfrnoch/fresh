...
pub enum Sndr<'a> {
  ...
  /// The `Misc` variant represents information that the client may want to display in a structured manner. The client can either implement its own way of displaying the information or use the provided `.alt` field.
  Misc {
    what: &'a str,
    data: &'a [&'a str],
    alt: &'a str,
